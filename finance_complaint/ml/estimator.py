...
from finance_complaint.logger import *
...
class S3Estimator(CloudEstimator):
    ...
    def __init__(self, bucket_name, region_name="ap-south-1", **kwargs):
        ...
        # logger.debug(f"All bucket response{response}")
        ...
        if bucket_name not in available_buckets:
            ...
            logger.info(f"Bucket not available. Creating bucket{bucket_name} at {location}")
...
class FinanceComplaintEstimator:
    # Deals with local model storage
