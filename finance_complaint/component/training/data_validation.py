...
class DataValidation(FinanceDataSchema):
    ...
    def get_unwanted_and_high_missing_value_columns(self, dataframe: DataFrame, threshold: float = 0.9) -> List[str]:
        try:
            ...
            logger.info(f"Unwanted Columns: {unwanted_column}")
    ...
    def is_required_columns_exist(self, dataframe: DataFrame):
        try:
            ...
            if len(columns) != len(self.schema.required_columns):
                ...
                logger.info(f"All required column exists")
        except Exception as e:
            raise FinanceException(e, sys)
